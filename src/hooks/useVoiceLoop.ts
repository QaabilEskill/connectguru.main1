...
type PreparedTtsAudio = {
  ...
  durationMs: number;
  createdAt: number;
  // Pre-warmed <audio> element. We create it inside prepareTtsAudio (during
  // prefetch or the first-beat fetch) and call load() so the browser starts
  // buffering/decoding the blob immediately, in parallel with whatever audio
  // is currently playing. By the time speak() reaches .play(), the element
  // is already ready — no extra buffering pause before the first sentence
  // and no gap between consecutive teaching beats.
...
export function useVoiceLoop(opts: {
  ...
  /**
   * Chapter day number (1-based). When 1, static teaching beats route to the
   * server-side MP3 cache (Supabase Storage) instead of regenerating audio
   * every session. Dynamic content (corrections, doubts) is not affected.
   */
  ...
  // Mirror dayNumber into a ref so prepare/stream callbacks can read the
  // latest value without changing identity. Day 1 → use cached MP3 path,
  // skip the live PCM streaming endpoint (which can't be cached).
  const dayNumberRef = useRef<number | null>(opts.dayNumber ?? null);
  useEffect(() => { dayNumberRef.current = opts.dayNumber ?? null; }, [opts.dayNumber]);
  const isDay1Cacheable = () => dayNumberRef.current === 1;
  ...
  const cleanup = useCallback(() => {
    ...
    // Cancel any browser SpeechSynthesis utterances just in case (twice — some
    // mobile browsers race the first cancel against a freshly queued utterance).
    try { window.speechSynthesis?.cancel(); } catch { /* noop */ }
    // Immediately silence the streaming gain so any sample already in the
    // audio context's render quantum is muted before we tear the graph down.
    try { ttsStreamGainRef.current?.gain.setValueAtTime(0, ttsStreamGainRef.current.context.currentTime); } catch { /* noop */ }
    try { ttsStreamGainRef.current?.disconnect(); } catch { /* noop */ }
    ttsStreamGainRef.current = null;
    // Suspend before close so output stops on the same tick (close() is async
    // on some browsers and can let one more buffer flush to the speakers).
    ...
    allTtsAudioElsRef.current.forEach((el) => {
      ...
      try { el.muted = true; el.volume = 0; } catch { /* noop */ }
    ...
    // Belt-and-suspenders: also pause any <audio>/<video> element that may
    // have been attached to the DOM (e.g. media-session silent loop) so
    // absolutely nothing can keep emitting sound after Exit.
    try {
      document.querySelectorAll('audio, video').forEach((node) => {
        const m = node as HTMLMediaElement;
        try { m.pause(); } catch { /* noop */ }
        try { m.muted = true; } catch { /* noop */ }
      });
    } catch { /* noop */ }
    preparedTtsUrlsRef.current.forEach((url) => { try { URL.revokeObjectURL(url); } catch { /* noop */ } });
    preparedTtsUrlsRef.current.clear();
    // Second speechSynthesis cancel after element teardown.
    try { window.speechSynthesis?.cancel(); } catch { /* noop */ }
  ...
  const prepareTtsAudio = useCallback(async (rawText: string, signal?: AbortSignal): Promise<PreparedTtsAudio | null> => {
    ...
    try {
      const doFetch = () => fetch(`${SUPA_FN}/tutor-tts`, {
        ...
        body: JSON.stringify({ text, strict: true, cacheable: isDay1Cacheable() }),
        signal: ac.signal,
      });
      let res: Response;
      try {
      ...
      const cacheStatus = (res.headers.get('X-TTS-Cache') ?? 'bypass').toLowerCase();
      let spoken = text;
      if (spokenB64) {
      ...
      if (stoppedRef.current || pausedRef.current || ac.signal.aborted) return null;
      // Verbose, unmissable log so we can confirm Day 1 is actually serving
      // from the stored MP3 (Supabase Storage) rather than re-hitting the
      // upstream TTS API. cacheStatus values:
      //   hit    → served from Supabase Storage MP3 (no TTS call)
      //   miss   → first generation; MP3 just written to Storage
      //   bypass → live TTS, no cache involved (dynamic content)
      const playbackSource = cacheStatus === 'hit'
        ? '✅ Stored MP3 (Supabase Storage cache)'
        : cacheStatus === 'miss'
          ? '⚙️  Live TTS (cache MISS — MP3 now stored for next time)'
          : '❌ Live TTS (no cache — dynamic content)';
      // eslint-disable-next-line no-console
      console.log(`[tutor-tts] Playback Source: ${playbackSource} | day=${dayNumberRef.current ?? 'n/a'} | bytes=${sourceBlob.size} | fetch=${Math.round(performance.now() - startedAt)}ms`);

      // ── Fast path for cache HIT ───────────────────────────────────────────
      // The cached MP3 was already produced by Ash with zero opening silence
      // and tight inter-sentence beats (server INSTRUCTIONS). Skipping the
      // decode → analyse → re-encode WAV round-trip eliminates ~150-400ms of
      // CPU work per beat on mobile and lets the <audio> element start
      // playing as soon as it has a chunk buffered.
      const skipOptimize = cacheStatus === 'hit';
      const optimized = skipOptimize
        ? { blob: sourceBlob, optimized: false, durationMs: 0 }
        : await optimizeTtsBlob(sourceBlob);
      if (stoppedRef.current || pausedRef.current || ac.signal.aborted) return null;
      const url = URL.createObjectURL(optimized.blob);
      preparedTtsUrlsRef.current.add(url);
      // Pre-warm the <audio> element here, while the previous beat (if any)
      // is still playing. Creating the element + setting preload='auto' +
      // calling load() kicks the browser into buffering/decoding the blob
      // immediately. By the time speak() reaches el.play(), playback can
      // start with zero buffering delay — this is what makes Day 1 feel
      // instant and removes the small gaps between consecutive sentences.
      let preWarmedEl: HTMLAudioElement | undefined;
      try {
        const el = new Audio();
        ...
        // Track immediately so cleanup() can tear it down even if speak() is
        // never reached (e.g. user hit Exit between prefetch and playback).
        allTtsAudioElsRef.current.add(el);
        try { el.load(); } catch { /* noop */ }
        preWarmedEl = el;
      } catch { /* noop — speak() will fall back to constructing a fresh Audio */ }
      try {
        opts.onDebug?.({
          ttsMs: String(Math.round(performance.now() - startedAt)),
          ttsOptimized: optimized.optimized ? 'yes' : skipOptimize ? 'cache-hit' : 'no',
          ttsDurationMs: String(Math.round(optimized.durationMs)),
          ttsCache: cacheStatus,
        });
      } catch { /* noop */ }
      return {
        text,
  ...
  const speak = useCallback(async (rawText: string, options?: { force?: boolean; prepared?: PreparedTtsAudio | Promise<PreparedTtsAudio | null> | null; onStarted?: () => void }): Promise<boolean> => {
    ...
    try {
      // Day 1 routes exclusively through the cached MP3 (prepareTtsAudio) so
      // every static teaching beat is served from Supabase Storage after the
      // first generation. Live PCM streaming bypasses the cache, so we skip
      // it entirely on Day 1.
      if (!options?.prepared && !isDay1Cacheable()) {
      ...
      if (!audioCompleted && prepared) {
        ...
        // Reuse the pre-warmed element from prepareTtsAudio when present —
        // it has already started buffering/decoding while the previous beat
        // was playing, so .play() returns immediately. Falls back to a
        // fresh Audio if for any reason no element was attached.
        const el = prepared.element ?? new Audio(url);
        (el as any).playsInline = true;
        el.setAttribute('playsinline', '');
        el.preload = 'auto';
        el.volume = 1.0;
