...
Return ONLY valid JSON:
{
  ...
  "chapterComplete": true | false
    ...
    // Award XP when the student just answered a real question correctly.
    // Prev assistant beat must have been a real question awaiting a response,
    // the current turn must include a userMessage, and the model marks it correct.
    try {
      const prevWasQuestion = (lastCorr?.beatType === 'question') && (lastCorr?.awaitingStudentResponse === true);
      if (userMessage && prevWasQuestion && correction.studentAnswerCorrect && !correction.needsRepeat) {
        const { data: cfg } = await sb.from('tutor_xp_config').select('value').eq('key', 'correct_answer_xp').maybeSingle();
        const amount = Number.isFinite(Number(cfg?.value)) ? Number(cfg?.value) : 20;
        if (amount > 0) await sb.rpc('increment_user_xp', { p_user_id: userId, p_amount: amount });
      }
    } catch (_) { /* XP is best-effort; never block the lesson */ }
