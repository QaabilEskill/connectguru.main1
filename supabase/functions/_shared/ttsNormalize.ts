...
const WORD_FIXES: Array<[string, string]> = [
  ...
  // ---- Indian-English / Hinglish question + function words ----
  // These are the words users reported sounding "foreign". The respellings
  // force George (English neural voice) into Indian-English phonemes:
  //  - "ai" -> "eye" diphthong (kaise = "kay-say", NOT "kayz")
  //  - short "a" -> "uh" schwa  (kya = "kyaa", kab = "kub")
  //  - retroflex "t/d" approximated by doubling the vowel
  ['kaise', 'kay-say'],
  ['kaisa', 'kay-saa'],
  ['kaisi', 'kay-see'],
  ['kya', 'kyaa'],
  ...
  // Classroom courtesy
  ['suniye', 'soo-nee-yay'],
  ['dekhiye', 'day-khee-yay'],
  ['samajhiye', 'suh-muh-jhee-yay'],
  ['samjho', 'sum-jho'],
  ['samjha', 'sum-jhaa'],
  ['batao', 'buh-taa-oh'],
  ['bataiye', 'buh-taa-ee-yay'],
  ['sahi hai', 'suh-hee hay'],
  ['theek hai', 'theek hay'],
  ['chaliye', 'chuh-lee-yay'],
  ['chalo', 'chuh-lo'],
];
...
export function normalizeForTts(text: string, gender: TutorVoiceGender = 'male'): string {
  ...
  // Pacing: ElevenLabs inserts a long pause for every ellipsis, double
  // period, or newline. Collapse those so George flows naturally between
  // sentences without sounding like stitched clips.
  s = s
    .replace(/\.{2,}/g, '.')     // "..." -> "."
    .replace(/\s*\n+\s*/g, ' ')  // line breaks -> single space
    .replace(/([.!?])\s{2,}/g, '$1 ') // collapse multi-space after sentence end
    .replace(/ +/g, ' ')
    .trim();
  return s;
}
